...
describe User do
  ...
  describe "abilities" do
    ...
    xcontext "guest" do
      let(:user) { FactoryGirl.create :user, guest: true }
      subject(:ability){ Ability.new(user) }

      it { expect(subject).to be_able_to(:read, Book.new) }
      it { expect(subject).not_to be_able_to(:new_rating, Book.new) }
      ...
      it { expect(subject).not_to be_able_to(:create, Rating.new(user: user)) }
